...
typedef struct mat4 {
  ...
  mat4(GLfloat p0, GLfloat p1, GLfloat p2, GLfloat p3, GLfloat p4, GLfloat p5, GLfloat p6, GLfloat p7, GLfloat p8,
       GLfloat p9, GLfloat p10, GLfloat p11, GLfloat p12, GLfloat p13, GLfloat p14, GLfloat p15) {
    m[0] = p0;
    m[1] = p1;
    m[2] = p2;
    m[3] = p3;
...
typedef struct mat3 {
  ...
  mat3(GLfloat p0, GLfloat p1, GLfloat p2, GLfloat p3, GLfloat p4, GLfloat p5, GLfloat p6, GLfloat p7, GLfloat p8) {
    m[0] = p0;
    m[1] = p1;
    m[2] = p2;
    m[3] = p3;
...
mat3 SetMat3(GLfloat p0, GLfloat p1, GLfloat p2, GLfloat p3, GLfloat p4, GLfloat p5, GLfloat p6, GLfloat p7,
             GLfloat p8);
mat4 SetMat4(GLfloat p0, GLfloat p1, GLfloat p2, GLfloat p3, GLfloat p4, GLfloat p5, GLfloat p6, GLfloat p7, GLfloat p8,
             GLfloat p9, GLfloat p10, GLfloat p11, GLfloat p12, GLfloat p13, GLfloat p14, GLfloat p15);
...
mat4 Mult(mat4 a, mat4 b);  // dest = a * b - rename to MultMat4 considered but I don't like to make the name of the
                            // most common operation longer
...
mat4 lookAt(GLfloat px, GLfloat py, GLfloat pz, GLfloat lx, GLfloat ly, GLfloat lz, GLfloat vx, GLfloat vy, GLfloat vz);
...
inline vec3 operator-(const vec3 &a) { return SetVector(-a.x, -a.y, -a.z); }
...
// mat4 * mat4
inline mat4 operator*(const mat4 &a, const mat4 &b) { return (Mult(a, b)); }

// mat3 * mat3
inline mat3 operator*(const mat3 &a, const mat3 &b) { return (MultMat3(a, b)); }
...
mat4 IdentityMatrix() {
  ...
  for (i = 0; i <= 15; i++) m.m[i] = 0;
  for (i = 0; i <= 3; i++) m.m[i * 5] = 1;  // 0,5,10,15
  return m;
}
...
mat4 Mult(mat4 a, mat4 b)  // m = a * b
{
  ...
  for (x = 0; x <= 3; x++)
    for (y = 0; y <= 3; y++)
      if (transposed)
        m.m[x * 4 + y] = a.m[y + 4 * 0] * b.m[0 + 4 * x] + a.m[y + 4 * 1] * b.m[1 + 4 * x] +
                         a.m[y + 4 * 2] * b.m[2 + 4 * x] + a.m[y + 4 * 3] * b.m[3 + 4 * x];
      else
        m.m[y * 4 + x] = a.m[y * 4 + 0] * b.m[0 * 4 + x] + a.m[y * 4 + 1] * b.m[1 * 4 + x] +
                         a.m[y * 4 + 2] * b.m[2 * 4 + x] + a.m[y * 4 + 3] * b.m[3 * 4 + x];
...
mat3 MultMat3(mat3 a, mat3 b)  // m = a * b
{
  ...
  for (x = 0; x <= 2; x++)
    for (y = 0; y <= 2; y++)
      if (transposed)
        m.m[x * 3 + y] =
            a.m[y + 3 * 0] * b.m[0 + 3 * x] + a.m[y + 3 * 1] * b.m[1 + 3 * x] + a.m[y + 3 * 2] * b.m[2 + 3 * x];
      else
        m.m[y * 3 + x] =
            a.m[y * 3 + 0] * b.m[0 * 3 + x] + a.m[y * 3 + 1] * b.m[1 * 3 + x] + a.m[y * 3 + 2] * b.m[2 * 3 + x];
...
mat4 MatrixAdd(mat4 a, mat4 b) {
  ...
  int i;
  for (i = 0; i < 16; i++) dest.m[i] = a.m[i] + b.m[i];
...
// 1 = column-wise
void SetTransposed(char t) { transposed = t; }
...
mat4 lookAtv(vec3 p, vec3 l, vec3 v) {
  ...
  if (transposed)
    rot = SetMat4(u.x, v.x, n.x, 0, u.y, v.y, n.y, 0, u.z, v.z, n.z, 0, 0, 0, 0, 1);
  else
    rot = SetMat4(u.x, u.y, u.z, 0, v.x, v.y, v.z, 0, n.x, n.y, n.z, 0, 0, 0, 0, 1);
  trans = T(-p.x, -p.y, -p.z);
  mat4 m = Mult(rot, trans);
  return m;
}

mat4 lookAt(GLfloat px, GLfloat py, GLfloat pz, GLfloat lx, GLfloat ly, GLfloat lz, GLfloat vx, GLfloat vy,
            GLfloat vz) {
  vec3 p, l, v;
...
mat4 perspective(float fovyInDegrees, float aspectRatio, float znear, float zfar) {
  ...
  mat4 m = SetMat4(f / aspectRatio, 0, 0, 0, 0, f, 0, 0, 0, 0, (zfar + znear) / (znear - zfar),
                   2 * zfar * znear / (znear - zfar), 0, 0, -1, 0);
  if (transposed) m = transpose(m);
  return m;
}
...
mat4 frustum(float left, float right, float bottom, float top, float znear, float zfar) {
  ...
  if (!transposed) matrix = Transpose(matrix);
...
mat4 ortho(GLfloat left, GLfloat right, GLfloat bottom, GLfloat top, GLfloat near, GLfloat far) {
  ...
  mat4 o = SetMat4(a, 0, 0, tx, 0, b, 0, ty, 0, 0, c, tz, 0, 0, 0, 1);

  if (transposed) o = Transpose(o);
...
mat3 InvertMat3(mat3 in) {
  ...
  if (DET != 0) {
    ...
    nanout = SetMat3(NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN);
    out = nanout;
  }
...
mat4 InvertMat4(mat4 a) {
  ...
  float c = a.m[0], d = a.m[1], e = a.m[2], g = a.m[3], f = a.m[4], h = a.m[5], i = a.m[6], j = a.m[7], k = a.m[8],
        l = a.m[9], o = a.m[10], m = a.m[11], n = a.m[12], p = a.m[13], r = a.m[14], s = a.m[15], A = c * h - d * f,
        B = c * i - e * f, t = c * j - g * f, u = d * i - e * h, v = d * j - g * h, w = e * j - g * i,
        x = k * p - l * n, y = k * r - o * n, z = k * s - m * n, C = l * r - o * p, D = l * s - m * p,
...
void printVec3(vec3 in) { printf("(%f, %f, %f)\n", in.x, in.y, in.z); }
...
#ifdef __cplusplus
mat3 inverse(mat3 m) { return InvertMat3(m); }
mat4 inverse(mat4 m) { return InvertMat4(m); }
mat3 transpose(mat3 m) { return TransposeMat3(m); }
mat4 S(GLfloat s) { return S(s, s, s); }
mat4 S(vec3 s) { return S(s.x, s.y, s.z); }
mat4 lookAt(vec3 p, vec3 l, vec3 u) { return lookAtv(p, l, u); }
