...
void glutReshapeFunc(void (*func)(int width, int height)) { gReshape = func; }
void glutDisplayFunc(void (*func)(void)) { gDisplay = func; }
void glutKeyboardFunc(void (*func)(unsigned char key, int x, int y)) { gKey = func; }
  ...
  if (chars != NULL) {
    ...
    switch ([theEvent keyCode]) {
      ...
      case 0x30:
        ...
        break;
      case 0x24:
        code = GLUT_KEY_RETURN;
        break;
      case 0x31:
        code = GLUT_KEY_SPACE;
        break;
      case 0x29:
        code = GLUT_KEY_SEMICOLON;
        break;
      case 0x2B:
        code = GLUT_KEY_COMMA;
        break;
      case 0x2F:
        code = GLUT_KEY_DECIMAL;
        break;
      case 0x32:
        code = GLUT_KEY_GRAVE;
        break;
      case 0x27:
        ...
        break;
      case 0x21:
        code = GLUT_KEY_LBRACKET;
        break;
      case 0x1E:
        code = GLUT_KEY_RBRACKET;
        break;
      case 0x2A:
        code = GLUT_KEY_BACKSLASH;
        break;
      case 0x2C:
        code = GLUT_KEY_SLASH;
        break;
      ...
      case 115:
        ...
        break;  // ?
      case 116:
        code = GLUT_KEY_PAGE_UP;
        break;
      case 119:
        code = GLUT_KEY_END;
        break;  // ?
      case 121:
        code = GLUT_KEY_PAGE_DOWN;
        break;
      case 117:
        code = GLUT_KEY_INSERT;
        break;  // Looks more like DEL?
      default:
    ...
    // NOTE: This was a bug until I modified the special key constants! We can now check gKeymap with normal ASCII and
    // special codes with the same table!
...
- (void)mouseDown:(NSEvent *)theEvent {
  ...
  if (gMouseFunc) gMouseFunc(0, GLUT_DOWN, p.x, p.y);
}
...
- (void)mouseUp:(NSEvent *)theEvent {
  ...
  p = [theEvent locationInWindow];
  p = [self convertPoint:p fromView:nil];
  if (gMouseFunc) gMouseFunc(0, GLUT_UP, p.x, p.y);
}
...
- (void)mouseDragged:(NSEvent *)theEvent {
  ...
  p = [theEvent locationInWindow];
  p = [self convertPoint:p fromView:nil];
  if (gMouseDragged) gMouseDragged(p.x, p.y);
}
...
- (void)mouseMoved:(NSEvent *)theEvent {
  ...
  p = [theEvent locationInWindow];
  p = [self convertPoint:p fromView:nil];
  if (gMouseMoved) gMouseMoved(p.x, p.y);
}
...
- (void)windowDidResize:(NSNotification *)note {
  ...
  if (gReshape) gReshape([theView frame].size.width, [theView frame].size.height);
}
...
@private
 ...
 void glutTimerFunc(int millis, void (*func)(int arg), int arg) {
   printf("glutTimerFunc\n");
   TimerInfoRec *timerInfo = [TimerInfoRec alloc];
   timerInfo->arg = arg;
   timerInfo->func = func;

   gTimer = [NSTimer scheduledTimerWithTimeInterval:millis / 1000.0
 ...
 int glutCreateWindow(const char *windowTitle)  // and window and view
 {
   ...
   if (!NSApp) NSApp = [NSApplication sharedApplication];
   [NSApp setActivationPolicy:NSApplicationActivationPolicyRegular];
   //    NSRect frame = NSMakeRect( gWindowInitPositionX, gWindowInitPositionY, gWindowInitWidth, gWindowInitHeight );
   NSRect frame = NSMakeRect(gWindowInitPositionX,
                             NSScreen.mainScreen.frame.size.height - gWindowInitPositionY - gWindowInitHeight,
                             gWindowInitWidth, gWindowInitHeight);
   ...
   [window setTitle:[[NSString alloc] initWithCString:windowTitle encoding:NSMacOSRomanStringEncoding]];
 ...
 void glutPostRedisplay() { [theView setNeedsDisplay:YES]; }
 ...
 char glutMouseIsDown(unsigned char c) { return gButtonPressed[(unsigned int)c]; }
 ...
 // This call is not in the original GLUT but IMHO better than relying on glutReshapeWindow or glutPositionWindow to
 // exit! (These calls also exit full screen mode though.)
 void glutExitFullScreen() {
   if (gFullScreen != 0) {
     gFullScreen = 0;
     [NSMenu setMenuBarVisible:TRUE];
     //		[window setStyleMask: NSTitledWindowMask | NSClosableWindowMask | NSMiniaturizableWindowMask |
     // NSResizableWindowMask]; // Back to normal
     [window setStyleMask:NSWindowStyleMaskTitled | NSWindowStyleMaskClosable | NSWindowStyleMaskMiniaturizable |
                          NSWindowStyleMaskResizable];  // Back to normal
